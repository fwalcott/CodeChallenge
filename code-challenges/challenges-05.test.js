...
const templateWithJQuery = () => {
  // Solution code here... 
  const $templateWithJQuery = $('#template'); 
  starWarsPeople.forEach(value => {
    let $newPeople = $template.clone(); 
    $newPeople.children('h2').text.(value.name); 
    $newPeople.children('h3').text.(value.height);
    $newPeople.children('p').text.(value.eye_color); 
...
const howMuchPencil = (str) => {
  let result = [];
  // Solution code here... 
  for (let i = 0; <str.lenght + 1; i++) {
    result.push(str.slice (i, str.lenght)); 
  } 
  return result;
};
