...
reponse.status(200).send('Hello');
  ...
  app.get('/aboutme', (request, response) =>{
    response.status(200).send('My name is Matrix and I am lost in the web. I am trying to get home to Mainframe and stop Megabyte');
  }); 

  app.get('/favoritefoods', (request, response) => {
    response.status(200).send('Do I eat food. I forgot. I should ask Dot or Bob if we do?');
  });
app.get('*', (request, response) => {
  response.status(404).send('Okay this is not right. I think we took a wrong turn somewhere');
});
